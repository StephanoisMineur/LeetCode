...
// dp[i]表示以nums[i]结尾的连续递增子序列长度。故需额外int保存最大值
// 默认初始长度均初始化为1
// 递推公式，比较相邻元素，nums[i]>nums[i-1]则dp[i]=dp[i-1]+1。
// 因为连续递增子序列，只和前一个元素有关，而不最长递增子序列，和前面所有元素都有关
...
// 对比相邻元素，如果nums[i]>nums[i-1]则count++，否则就返回count=1。记录最大值
