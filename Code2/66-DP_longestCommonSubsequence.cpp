...
// 最长公共子序列，要求可以不连续，故dp[i][j]表示以text1[i-1]和text2[j-1]结尾的子序列，公共最长值
// 递推公式，如果两个末尾相等，则dp[i][j]=dp[i-1][j-1]+1。
// 如果末尾元素不相等，则延续最大值dp[i][j]=max(dp[i-1][j],dp[i][j-1])
...
// 一维数组写法。由于dp[i][j]需要依赖上、左、左上三个方向的值，故遍历方向只能从左至右。
// 使用pre保存左上值
