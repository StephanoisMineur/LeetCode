...
深拷贝应该正好由 n 个 全新节点组成，
其中每个新节点的值都设为其对应的原节点的值。新节点的 next 指针和
random指针也都应指向复制链表中的新节点，并使原链表和复制链表中的这些指针
...
// 方法一，外置哈希记录老、新结点对。递归退出条件，老结点为空返回空。
// 检测该层老结点对应的是否创建过，如无，创建新结点
// 注意，哈希必须先记录新结点，再回溯对新结点的next/random按照老结点的next/random的赋值。
// 否则递归时永远找不到新结点，无限创建。时间、空间复杂度均为O(1)
...
// 方法二，迭代结点拆分，三次遍历，一次复制A->A'，二次赋值A'的所有random结点。
// 三次拆分A和A'，要先排除传入指针为空的情况
// 二次遍历赋值random时，注意考虑原random为空指针的特殊情况。
// 三次遍历剪切时，考虑q是否是最后一个结点，否则为空
