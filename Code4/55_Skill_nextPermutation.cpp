...
// 不要使用回溯全排列。实质是找组成整数紧接着更大的一个。
// 第一步，从后向前，找第一个升序数对a[i]<a[i+1]。必须严格小于
// 具体操作是先初始化i=n-2，如果a[i]>=a[i+1]就不断前移，注意等号条件
// 找[i+1,n)区间内（必为降序），第一次出现a[j]>a[i]的位置，交换i和j。
// 对[i+1,n)区间重新按从小到大排序。注意，如果i==-1说明一直没有找到，说明为全降序，直接反转序列
// 时间复杂度O(n)，因为至多扫描两遍序列，一遍反转，且原地修改。
/// 空间复杂度O(1)，此函数为C++标准库函数next_permutation
...
// 数组是中间高两边低的尖峰型，核心是找到第一个左侧nums[i]小于右侧nums[j]的对，二者置换
// 然后对置换后[i+1,n)区间反转重排，注意反转即为排序，因为右侧必然是降序
