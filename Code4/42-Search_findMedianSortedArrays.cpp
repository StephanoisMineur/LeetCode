...
// 辅助函数为求第k个最小元素，传入参数为两数组，各个起始、终止点，当前剩余k。
// 首先求两数组长度，利用递归参数交换，保证len1<len2
// 如果len1已经为0，返回len2有效区间的第k小的元素。
// 如果k==1，即求len1和len2两个有效区间首元素最小值
// 以长度k/2为截断点，注意排除其长度可能已经超过Len1或len2的意外。
// 两截断点值nums1[i]和nums2[j]更小的一个，前方舍弃，start从i+1，k长度亦更新
