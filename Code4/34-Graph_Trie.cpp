...
// 两个特征，vector存储26个孩子指针，isEnd存储是否为最后一个字符。
// insert函数将node初始化为当前层。对每word每个字符，如果没孩子，就创造孩子。
// 然后移动到孩子的位置上，如此node停于最后孩子，isEnd置真。
// 单独设置返回Trie*的search函数，node处于树上，对每一个word字符，找不到立刻返回空。
// 找到了就移动置孩子位的节点Trie*，最终返回孩子位节点Trie*。
class Trie1 {
 public:
  Trie1() {
    children.resize(26);
