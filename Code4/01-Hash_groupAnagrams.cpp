...
// 字母异位词是否同组，取决于其字母和数量是否严格相等。
// 方法一，可以通过哈希表map判断，方法二。可以通过重新排序后字符串相等判断
// 整体哈希表的键，为该组字符串的特殊标识。然而，umap的键不能是另一个umap
// 故只能使用方法二，利用排序后的临时字符串作为键key
// 哈希表值val，为该组下的所有原始字符串。一次遍历，得到整体哈希表。
// 二次遍历整体哈希表，收集每一个“值”压入数组即可作为结果
