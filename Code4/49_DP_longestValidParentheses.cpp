...
// 首选利用栈，栈内存储定义是，最后一个未匹配的')'下标，其实质是截断点。
// 为避免首元素为'('导致无右括号特殊情况。栈内先压入一个-1.
// 遍历元素，如果为'('，压入栈下标。如果为')'，先弹栈首，作为(的配对，或)的更新。
// 若此时栈已空，则说明该)未匹配，下标压入栈内。
...
// 时间复杂度O(n),空间复杂度O(1)。贪心，左右两遍遍历，记录(、)分别出现次数
// 若相等就求长度，左向右遍历时，若right>left则截断，计数归零
